...
public class SpatialAudioSetter : MonoBehaviour
{
	[SerializeField] float radius;

	PhotonView PV;

	static Dictionary<Player, SpatialAudioSetter> spatialAudioFromPlayers = new Dictionary<Player, SpatialAudioSetter>();
	...
	void Awake()
	{
		...
		agoraAudioEffects = AgoraManager.Instance.GetRtcEngine().GetAudioEffectManager();
	...
	void Update()
	{
		...
		foreach(Player player in PhotonNetwork.CurrentRoom.Players.Values)
		{
			...
			if(player.CustomProperties.TryGetValue("agoraID", out object agoraID))
			{
				...
				{
					SpatialAudioSetter other = spatialAudioFromPlayers[player];
